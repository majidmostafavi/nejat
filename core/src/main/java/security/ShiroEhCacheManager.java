...
/**
 * Created by majidmostafavi on 10/4/15.
 */
public class ShiroEhCacheManager implements CacheManager, Initializable, Destroyable {
    ...
    protected InputStream getCacheManagerConfigFileInputStream() {
        ...
        try {
            ...
            throw new IllegalStateException("Unable to obtain input stream for cacheManagerConfigFile [" + configFile + "]", var3);
        }
    }
    ...
    private net.sf.ehcache.CacheManager ensureCacheManager() {
        try {
            if(this.manager == null) {
                List e = net.sf.ehcache.CacheManager.ALL_CACHE_MANAGERS;
                if(e.size() > 0) {
                    this.manager = (net.sf.ehcache.CacheManager)e.get(0);
                } else {
    ...
    public void destroy() {
        if(this.cacheManagerImplicitlyCreated) {
            try {
                net.sf.ehcache.CacheManager t = this.getCacheManager();
                t.shutdown();
